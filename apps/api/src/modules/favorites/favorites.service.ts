...
export class FavoritesService {
  ...
  async listStickerFavorites(userId: string) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { stickerFavoriteIds: true }
    });
    if (!user) throw new NotFoundException('User was not found.');
    return user.stickerFavoriteIds;
  }

  async addStickerFavorite(userId: string, stickerIdInput: string) {
    const stickerId = this.normalizeStickerId(stickerIdInput);
    const current = await this.listStickerFavorites(userId);
    const stickerFavoriteIds = Array.from(new Set([...current, stickerId])).slice(-300);
    const user = await this.prisma.user.update({
      where: { id: userId },
      data: { stickerFavoriteIds },
      select: { stickerFavoriteIds: true }
    });
    return user.stickerFavoriteIds;
  }

  async removeStickerFavorite(userId: string, stickerIdInput: string) {
    const stickerId = this.normalizeStickerId(stickerIdInput);
    const current = await this.listStickerFavorites(userId);
    const user = await this.prisma.user.update({
      where: { id: userId },
      data: { stickerFavoriteIds: current.filter((item) => item !== stickerId) },
      select: { stickerFavoriteIds: true }
    });
    return user.stickerFavoriteIds;
  }
  ...
  private normalizeStickerId(stickerIdInput: string) {
    const stickerId = String(stickerIdInput ?? '').trim();
    if (!/^[a-z0-9][a-z0-9._-]{1,119}$/i.test(stickerId)) {
      throw new BadRequestException('Invalid sticker id.');
    }
    return stickerId;
  }
